...
void print_task_list_help() {
    std::cout
        ...
        << "    --build-info-json  Print the FishS2RT protocol and capabilities as JSON\n"
        ...
        << "  Batch:\n"
        << "    --request-sequence <json>  Run JSON requests in one persistent offline or streaming session\n"
        << "    --request-loop  Keep one streaming model session alive and accept JSON commands on stdin\n"
...
minitts::app::AppBatchResult run_streaming_request_sequence(
    engine::runtime::IVoiceTaskSession & session,
    engine::runtime::IStreamingVoiceTaskSession & streaming,
    const minitts::app::AppBatchRequest & batch_request,
    const minitts::app::FileOutputPolicy & outputs,
    bool metrics_requested) {
    if (batch_request.requests.empty()) {
        throw std::runtime_error("streaming request sequence requires at least one request");
    }
    const auto session_start = std::chrono::steady_clock::now();
    const auto prepare_start = std::chrono::steady_clock::now();
    session.prepare(engine::runtime::build_preparation_request(batch_request.requests.front().request));
    minitts::app::AppBatchResult batch_result;
    batch_result.prepare_ms = duration_ms(std::chrono::steady_clock::now() - prepare_start);
    batch_result.results.reserve(batch_request.requests.size());
    for (size_t index = 0; index < batch_request.requests.size(); ++index) {
        const auto & item = batch_request.requests[index];
        const std::string safe_id = minitts::app::safe_output_name(item.id);
        size_t event_sequence = 0;
        const minitts::app::StreamEventSink sink = [&](const engine::runtime::StreamEvent & event) {
            engine::runtime::TaskResult event_result;
            if (event.audio_output.has_value()) {
                event_result.named_audio_outputs.push_back({
                    safe_id + "_stream_" + std::to_string(event_sequence++),
                    *event.audio_output,
                    {},
                });
            }
            for (const auto & output : event.named_audio_outputs) {
                auto named = output;
                named.id = safe_id + "_stream_" + std::to_string(event_sequence++);
                event_result.named_audio_outputs.push_back(std::move(named));
            }
            event_result.speaker_turns = event.speaker_turns;
            event_result.word_timestamps = event.word_timestamps;
            event_result.output_artifacts = event.output_artifacts;
        ...
        auto result = minitts::app::run_streaming_task(streaming, item.request, sink);
        const double wall_ms = duration_ms(std::chrono::steady_clock::now() - run_start);
        batch_result.results.push_back({item.id, std::move(result), wall_ms});
        minitts::app::emit_batch_item_result(index, batch_result.results.back(), outputs);
        if (metrics_requested) {
            minitts::app::emit_task_metrics(
                batch_result.results.back().result,
                audio_metrics_info(item.request),
                wall_ms,
                "metrics[" + safe_id + "]");
        }
        std::cout << std::flush;
    }
    batch_result.session_wall_ms = duration_ms(std::chrono::steady_clock::now() - session_start);
    minitts::app::emit_batch_summary(batch_result, outputs);
    std::cout << std::flush;
    return batch_result;
}

std::string single_line_error(std::string value) {
    for (char & ch : value) {
        if (ch == '\r' || ch == '\n' || ch == '\t') {
            ch = ' ';
        }
...
void run_streaming_request_loop(
    engine::runtime::IVoiceTaskSession & session,
    engine::runtime::IStreamingVoiceTaskSession & streaming,
    const engine::audio::WavWriteOptions & wav_options,
    bool metrics_requested) {
    std::cout << "request_loop_ready=1\n" << std::flush;
    std::string line;
    while (std::getline(std::cin, line)) {
        ...
        try {
            const auto command = engine::io::json::parse(line);
            if (!command.is_object()) {
                throw std::runtime_error("request-loop command must be a JSON object");
            }
            command_id = minitts::cli::json_optional_string(command, "id").value_or("unknown");
            const auto action = minitts::cli::json_optional_string(command, "command").value_or("");
            if (action == "quit") {
                std::cout << "request_loop_bye=1\n" << std::flush;
                return;
            }
            const auto sequence_path = minitts::cli::json_optional_string(command, "request_sequence");
            if (!sequence_path.has_value() || sequence_path->empty()) {
                throw std::runtime_error("request-loop command requires request_sequence");
            }
            auto batch_request = minitts::cli::load_request_sequence(*sequence_path);
            if (action == "prepare") {
                session.prepare(engine::runtime::build_preparation_request(batch_request.requests.front().request));
            } else if (action == "run") {
                const auto output_dir = minitts::cli::json_optional_string(command, "out_dir");
                if (!output_dir.has_value() || output_dir->empty()) {
                    throw std::runtime_error("request-loop run command requires out_dir");
                }
                const minitts::app::FileOutputPolicy outputs{
                    std::nullopt,
                ...
                (void) run_streaming_request_sequence(
                    session,
                    streaming,
                    batch_request,
                    outputs,
                ...
                throw std::runtime_error("request-loop command must be prepare, run, or quit");
            }
            std::cout << "request_loop_done=" << command_id << "\n" << std::flush;
        } catch (const std::exception & error) {
            streaming.reset();
            std::cout << "request_loop_error=" << command_id << "\t"
                      << single_line_error(error.what()) << "\n" << std::flush;
        }
    }
}
...
int audiocpp_cli_main(int argc, char ** argv) {
    try {
        ...
        const bool request_loop_requested = has_arg(argc, argv, "--request-loop");
        if (has_arg(argc, argv, "--version")) {
            minitts::app::print_build_info(std::cout);
            return 0;
        }
        if (has_arg(argc, argv, "--build-info-json")) {
            minitts::app::print_build_info_json(std::cout);
            return 0;
        }
        ...
        if (metrics_requested && task_spec.mode != engine::runtime::RunMode::Offline &&
            ...
            !request_loop_requested) {
            throw std::runtime_error("streaming --metrics currently requires --request-sequence or --request-loop");
        }
        ...
        if (request_loop_requested) {
            if (task_spec.mode != engine::runtime::RunMode::Streaming) {
                throw std::runtime_error("--request-loop requires --mode streaming");
            }
            if (stream_audio_from_stdin(argc, argv)) {
                throw std::runtime_error("--request-loop owns stdin and cannot be combined with --audio -");
            }
            if (has_batch_input(argc, argv)) {
                throw std::runtime_error("--request-loop cannot be combined with batch input options");
            }
            if (voice_state_out.has_value() || has_vad_chunk_option(argc, argv)) {
                throw std::runtime_error("--request-loop does not support voice-state or VAD chunk outputs");
            }
            ...
            std::cout << "task=" << engine::runtime::to_string(session->task_kind()) << "\n";
            std::cout << "mode=" << engine::runtime::to_string(session->run_mode()) << "\n";
            run_streaming_request_loop(*session, *streaming, wav_options, metrics_requested);
            return 0;
        }
        ...
        if (has_batch_input(argc, argv)) {
            ...
            std::cout << "mode=" << engine::runtime::to_string(session->run_mode()) << "\n";
            if (task_spec.mode == engine::runtime::RunMode::Streaming) {
                if (!optional_path_arg(argc, argv, "--request-sequence").has_value()) {
                    throw std::runtime_error("streaming batch execution currently requires --request-sequence");
                }
                if (merge_mode != minitts::app::AudioMergeMode::None || outputs.audio_out.has_value()) {
                    throw std::runtime_error("streaming request sequences write per-request outputs and do not support merged --out");
                }
                auto * streaming = dynamic_cast<engine::runtime::IStreamingVoiceTaskSession *>(session.get());
                if (streaming == nullptr) {
                    throw std::runtime_error("selected task session does not support streaming execution");
            ...
            if (task_spec.mode != engine::runtime::RunMode::Offline) {
                throw std::runtime_error("batch inputs require offline mode or a streaming request sequence");
            }
