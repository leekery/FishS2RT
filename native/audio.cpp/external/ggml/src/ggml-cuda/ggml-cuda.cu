...
ggml_backend_cuda_context::~ggml_backend_cuda_context() {
    ...
    if (bf16_p2_scratch) {
        ggml_cuda_set_device(device);CUDA_CHECK(cudaDeviceSynchronize());
#ifdef USE_CUDA_GRAPH
        cuda_graphs.clear();
#endif
        ggml_backend_buffer_free(bf16_p2_scratch);bf16_p2_scratch=nullptr;
    }
#endif
...
#ifdef GGML_CUDA_BF16_P2_AVAILABLE
    if (ggml_cuda_is_bf16_p2(src0)) {
        if (ggml_cuda_bf16_p2_supports(src0,src1,dst)) { ggml_cuda_bf16_p2_mul_mat(ctx,src0,src1,dst,0); return; }
        GGML_ASSERT(ggml_cuda_bf16_p2_can_materialize(src0,src1,dst));
        // Identical logical metadata and ordinary-buffer identity: invoke the
        // original selector/precision policy, without mutating the graph tensor.
        const ggml_tensor raw = ggml_cuda_bf16_p2_materialize(ctx,src0);
        ggml_cuda_mul_mat(ctx,&raw,src1,dst);return;
    }
#endif
    const bool split = ggml_backend_buft_is_cuda_split(src0->buffer->buft);
...
static void ggml_backend_cuda_set_tensor_async(ggml_backend_t backend, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    if (ggml_cuda_is_bf16_p2(tensor)) { ggml_backend_tensor_set(tensor, data, offset, size); return; }
    ggml_backend_cuda_context * cuda_ctx = (ggml_backend_cuda_context *) backend->context;
...
static void ggml_backend_cuda_get_tensor_async(ggml_backend_t backend, const ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    if (ggml_cuda_is_bf16_p2(tensor)) { ggml_backend_tensor_get(tensor, data, offset, size); return; }
    ggml_backend_cuda_context * cuda_ctx = (ggml_backend_cuda_context *) backend->context;
    ggml_backend_buffer_t buf = tensor->view_src ? tensor->view_src->buffer : tensor->buffer;
...
static void ggml_backend_cuda_set_tensor_2d_async(ggml_backend_t backend, struct ggml_tensor * tensor, const void * data,
        size_t offset, size_t size, size_t n_copies, size_t stride_tensor, size_t stride_data) {
    if (ggml_cuda_is_bf16_p2(tensor)) {
        for (size_t i=0;i<n_copies;++i) { ggml_backend_tensor_set(tensor, (const char *)data+i*stride_data, offset+i*stride_tensor, size); } return;
    }
    ggml_backend_cuda_context * cuda_ctx = (ggml_backend_cuda_context *) backend->context;
    ggml_backend_buffer_t buf = tensor->view_src ? tensor->view_src->buffer : tensor->buffer;
...
static void ggml_backend_cuda_get_tensor_2d_async(ggml_backend_t backend, const struct ggml_tensor * tensor, void * data,
        size_t offset, size_t size, size_t n_copies, size_t stride_tensor, size_t stride_data) {
    if (ggml_cuda_is_bf16_p2(tensor)) {
        for (size_t i=0;i<n_copies;++i) { ggml_backend_tensor_get(tensor, (char *)data+i*stride_data, offset+i*stride_tensor, size); } return;
    }
    ggml_backend_cuda_context * cuda_ctx = (ggml_backend_cuda_context *) backend->context;
    ggml_backend_buffer_t buf = tensor->view_src ? tensor->view_src->buffer : tensor->buffer;
...
#if !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA)
// Only a full-span, closed chain is eligible:
// BF16 MUL_MAT(F32 vector) -> [VIEW/RESHAPE]* -> ROUND_BF16(F32 output).
// In particular, packed QKV/FFN slices and any externally observed unrounded
// projection must fall back; rounding those entire projections is not this rule.
static int ggml_cuda_try_bf16_matvec_round(
        ggml_backend_cuda_context * ctx, ggml_cgraph * graph, int first) {
    static const bool disabled = getenv("GGML_CUDA_DISABLE_BF16_MATVEC_ROUND") != nullptr &&
        std::atoi(getenv("GGML_CUDA_DISABLE_BF16_MATVEC_ROUND")) != 0;
    static const int debug = getenv("GGML_CUDA_DEBUG_BF16_MATVEC_ROUND") != nullptr ?
        std::atoi(getenv("GGML_CUDA_DEBUG_BF16_MATVEC_ROUND")) : 0;
    if (disabled) {
        return 0;
    }
    const ggml_tensor * mm = graph->nodes[first];
    ...
    auto on_device = [&](const ggml_tensor * t) {
        return t && t->data && t->buffer &&
            (t->buffer->buft == ggml_backend_cuda_buffer_type(ctx->device) ||
             (t == weight && ggml_cuda_bf16_p2_on_device(t, ctx->device)));
    };
    if (weight->type != GGML_TYPE_BF16 || input->type != GGML_TYPE_F32 || mm->type != GGML_TYPE_F32 ||
        width <= 0 || width % 512 != 0 || width > INT_MAX || rows <= 0 || rows > INT_MAX/width ||
        weight->ne[2] != 1 || weight->ne[3] != 1 ||
        input->ne[0] != width || ggml_nrows(input) != 1 ||
        mm->ne[0] != rows || ggml_nrows(mm) != 1 ||
        !ggml_is_contiguous(weight) || !ggml_is_contiguous(input) || !ggml_is_contiguous(mm) ||
        !on_device(weight) || !on_device(input) || !on_device(mm) || mm->view_src ||
        (uintptr_t) weight->data % sizeof(uint32_t) != 0 ||
        (uintptr_t) input->data % sizeof(float2) != 0 ||
        ggml_cuda_info().devices[ctx->device].warp_size != 32 ||
        !(mm->flags & GGML_TENSOR_FLAG_COMPUTE)) {
        return 0;
    }
    if (!ggml_cuda_should_fuse_mul_mat_vec_f(mm)) {
        return 0;
    }
    ...
    // At most four metadata bridges. Every skipped node must be the next link
    // in this chain, not merely a node with compatible shape somewhere nearby.
    for (int last = first + 1; last < std::min(graph->n_nodes, first + 6); ++last) {
        ggml_tensor * out = graph->nodes[last];
        const ggml_tensor * previous = graph->nodes[last - 1];
        if (out->src[0] != previous || out->type != GGML_TYPE_F32 ||
            out->ne[0] != rows || ggml_nrows(out) != 1 || !ggml_is_contiguous(out) ||
            !on_device(out) || (uintptr_t) out->data % sizeof(float) != 0 ||
            (previous->flags & GGML_TENSOR_FLAG_OUTPUT) ||
            ggml_node_get_use_count(graph, last - 1) != 1) {
            return 0;
        }
        ...
        // An in-place final unary is safe only when its view base is also an
        // eliminated internal node. Never write externally visible state.
        for (const ggml_tensor * base = out->view_src; base; base = base->view_src) {
            bool internal = false;
            for (int k = first; k < last; ++k) {
                internal |= graph->nodes[k] == base;
            }
            if (!internal) {
                return 0;
            }
        }
        if (out->op == GGML_OP_VIEW || out->op == GGML_OP_RESHAPE) {
            if (out->data != mm->data || out->view_offs != 0 || ggml_nbytes(out) != ggml_nbytes(mm)) {
                return 0;
            }
            continue;
        }
        if (out->op != GGML_OP_UNARY || ggml_get_unary_op(out) != GGML_UNARY_OP_ROUND_BF16 ||
            !(out->flags & GGML_TENSOR_FLAG_COMPUTE)) {
            return 0;
        }
        // The generic validator intentionally ignores constants; include both
        // external operands here, with subtraction-based, overflow-safe ranges.
        auto overlaps_output = [&](const ggml_tensor * t) {
            const uintptr_t a = (uintptr_t) out->data;
            const uintptr_t b = (uintptr_t) t->data;
            return a <= b ? b - a < ggml_nbytes(out) : a - b < ggml_nbytes(t);
        };
        if (overlaps_output(weight) || overlaps_output(input)) {
            if (debug > 1) {
                GGML_LOG_INFO("BF16 matvec round fallback: width=%lld rows=%lld reason=external-input-alias\n",
                    (long long) width, (long long) rows);
            }
            return 0;
        }
        const int outputs[] = { last };
        if (!ggml_cuda_check_fusion_memory_ranges(graph, first, last - first + 1, outputs, 1)) {
            if (debug > 1) {
                GGML_LOG_INFO("BF16 matvec round fallback: width=%lld rows=%lld reason=memory-range\n",
                    (long long) width, (long long) rows);
            }
            return 0;
        }
        ggml_cuda_mul_mat_vec_bf16_round_output(*ctx, weight, input, out);
        if (debug) {
            static std::atomic<unsigned long long> hits{0};
            // Counts backend dispatch/capture hits, not CUDA graph replays.
            const auto hit = hits.fetch_add(1, std::memory_order_relaxed) + 1;
            GGML_LOG_INFO("BF16 matvec round fusion: dispatch_hit=%llu width=%lld rows=%lld nodes=%d\n",
                hit, (long long) width, (long long) rows, last - first + 1);
        }
        return last - first;
    }
    return 0;
...
// Recognize only the closed, single-token packed BF16 FFN subgraph:
//   mm = [gate; up] * x
//   R(R(silu(R(view_gate(mm)))) * R(view_up(mm)))
// where R is the explicit ROUND_BF16 unary op (F32 output). Metadata-only
// RESHAPE/VIEW nodes may intervene. Ordinary SwiGLU fusion is not equivalent.
static int ggml_cuda_try_bf16_rounded_swiglu(
        ggml_backend_cuda_context * ctx, ggml_cgraph * graph, int first) {
    static const bool disabled = getenv("GGML_CUDA_DISABLE_BF16_ROUNDED_SWIGLU") != nullptr &&
        std::atoi(getenv("GGML_CUDA_DISABLE_BF16_ROUNDED_SWIGLU")) != 0;
    if (disabled) {
        return 0;
    }
    ggml_tensor * mm = graph->nodes[first];
    if (mm->op != GGML_OP_MUL_MAT || !mm->src[0] || !mm->src[1]) {
        return 0;
    }
    const ggml_tensor * weight = mm->src[0];
    const ggml_tensor * input  = mm->src[1];
    const int64_t width = weight->ne[0];
    const int64_t rows = weight->ne[1]/2;
    if (weight->type != GGML_TYPE_BF16 || input->type != GGML_TYPE_F32 || mm->type != GGML_TYPE_F32 ||
        width <= 0 || width % 512 != 0 || rows <= 0 || weight->ne[1] != 2*rows ||
        width > INT_MAX || rows > INT_MAX/2/width || weight->ne[2] != 1 || weight->ne[3] != 1 ||
        input->ne[0] != width || ggml_nrows(input) != 1 ||
        mm->ne[0] != 2*rows || ggml_nrows(mm) != 1 ||
        !ggml_is_contiguous(weight) || !ggml_is_contiguous(input) || !ggml_is_contiguous(mm) ||
        !weight->buffer || !input->buffer ||
        (weight->buffer->buft != ggml_backend_cuda_buffer_type(ctx->device) && !ggml_cuda_bf16_p2_on_device(weight, ctx->device)) ||
        input->buffer->buft != ggml_backend_cuda_buffer_type(ctx->device) ||
        ggml_cuda_info().devices[ctx->device].warp_size != 32) {
        return 0;
    }

    auto unary_is = [](const ggml_tensor * t, ggml_unary_op op) {
        return t && t->op == GGML_OP_UNARY && ggml_get_unary_op(t) == op &&
            t->type == GGML_TYPE_F32 && t->src[0] && !t->src[1];
    };
    // This is intentionally a bounded local pattern, not an arbitrary graph
    // rewrite. Any extra consumer/output/effect causes the reference fallback.
    for (int last = first + 1; last < std::min(graph->n_nodes, first + 15); ++last) {
        ggml_tensor * out = graph->nodes[last];
        if (!unary_is(out, GGML_UNARY_OP_ROUND_BF16)) {
            continue;
        }
        const ggml_tensor * mul = out->src[0];
        if (mul->op != GGML_OP_MUL || mul->type != GGML_TYPE_F32) {
            continue;
        }
        const ggml_tensor * silu_round = mul->src[0];
        const ggml_tensor * up_round = mul->src[1];
        if (!unary_is(silu_round, GGML_UNARY_OP_ROUND_BF16) ||
            !unary_is(up_round, GGML_UNARY_OP_ROUND_BF16)) {
            continue;
        }
        const ggml_tensor * silu = silu_round->src[0];
        if (!unary_is(silu, GGML_UNARY_OP_SILU) ||
            !unary_is(silu->src[0], GGML_UNARY_OP_ROUND_BF16)) {
            continue;
        }
        const ggml_tensor * gate_round = silu->src[0];
        std::vector<const ggml_tensor *> members = { mm, gate_round, silu, silu_round, up_round, mul, out };
        auto add_member = [&](const ggml_tensor * t) {
            if (std::find(members.begin(), members.end(), t) == members.end()) {
                members.push_back(t);
            }
        };
        auto packed_slice = [&](const ggml_tensor * slice, size_t offset) {
            if (!slice || slice->type != GGML_TYPE_F32 || slice->ne[0] != rows ||
                ggml_nrows(slice) != 1 || !ggml_is_contiguous(slice) ||
                (const char *) slice->data != (const char *) mm->data + offset) {
                return false;
            }
            const ggml_tensor * cur = slice;
            for (int depth = 0; depth < 4 && cur != mm; ++depth) {
                if (!cur || (cur->op != GGML_OP_VIEW && cur->op != GGML_OP_RESHAPE) ||
                    cur->type != GGML_TYPE_F32 || !ggml_is_contiguous(cur) || cur->src[1]) {
                    return false;
                }
                add_member(cur);
                cur = cur->src[0];
        ...
        if (!packed_slice(gate_round->src[0], 0) ||
            !packed_slice(up_round->src[0], size_t(rows)*sizeof(float))) {
            continue;
        }
        bool valid = members.size() == size_t(last - first + 1);
        for (int k = first; valid && k <= last; ++k) {
            const ggml_tensor * t = graph->nodes[k];
            if (std::find(members.begin(), members.end(), t) == members.end() ||
                (k != last && (t->flags & GGML_TENSOR_FLAG_OUTPUT)) ||
                t->type != GGML_TYPE_F32 || !ggml_is_contiguous(t)) {
                valid = false;
                break;
            }
            if (t->op != GGML_OP_VIEW && t->op != GGML_OP_RESHAPE && t != mm &&
                (t->ne[0] != rows || ggml_nrows(t) != 1)) {
                valid = false;
                break;
            }
            ...
            // Views must alias only intermediates eliminated by this fusion;
            // do not silently eliminate a view into externally observed state.
            for (const ggml_tensor * base = t->view_src; base; base = base->view_src) {
                if (std::find(members.begin(), members.end(), base) == members.end()) {
                    valid = false;
                    break;
                }
            }
            ...
            if (k != last) {
                int internal_uses = 0;
                for (const ggml_tensor * consumer : members) {
                    for (int src = 0; src < GGML_MAX_SRC; ++src) {
                        internal_uses += consumer->src[src] == t;
                    }
                }
                valid = ggml_node_get_use_count(graph, k) == internal_uses;
            }
        }
        if (!valid || !out->buffer || out->buffer->buft != ggml_backend_cuda_buffer_type(ctx->device)) {
            continue;
        }
        const int outputs[] = { last };
        if (!ggml_cuda_check_fusion_memory_ranges(graph, first, last - first + 1, outputs, 1)) {
        ...
        // Check the external inputs explicitly, including constant weights.
        auto overlaps_output = [&](const ggml_tensor * t) {
            const uintptr_t a = (uintptr_t) out->data;
            const uintptr_t b = (uintptr_t) t->data;
            return a < b + ggml_nbytes(t) && b < a + ggml_nbytes(out);
        };
        if (overlaps_output(weight) || overlaps_output(input)) {
            continue;
        }
        ggml_cuda_mul_mat_vec_bf16_rounded_swiglu(*ctx, weight, input, out);
        static std::atomic<bool> reported{false};
        if (getenv("GGML_CUDA_DEBUG_BF16_ROUNDED_SWIGLU") && !reported.exchange(true)) {
            GGML_LOG_INFO("BF16 rounded SwiGLU fusion: width=%lld rows=%lld nodes=%d\n",
                (long long) width, (long long) rows, last - first + 1);
        }
        return last - first;
    }
    return 0;
...
#if !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA)
    if (const int skipped = ggml_cuda_try_bf16_rounded_swiglu(cuda_ctx, cgraph, i)) {
        return skipped;
    }
    if (const int skipped = ggml_cuda_try_bf16_matvec_round(cuda_ctx, cgraph, i)) {
        return skipped;
    }
#endif
    // Only the explicit exact C1/C2 rules above understand transformed weights.
    for (int src=0;src<GGML_MAX_SRC;++src) {
        if (ggml_cuda_is_bf16_p2(node->src[src])) { return 0; }
    }
...
static void ggml_cuda_graph_evaluate_and_capture(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, const bool use_cuda_graph, const bool cuda_graph_update_required, const void * graph_key) {
    ...
    bool                         p2_serial_prefill = false;
#ifdef GGML_CUDA_BF16_P2_AVAILABLE
    for (int i=0;i<cgraph->n_nodes;++i) {
        const auto node=cgraph->nodes[i];
        p2_serial_prefill |= node->op == GGML_OP_MUL_MAT && ggml_cuda_is_bf16_p2(node->src[0]) &&
            !ggml_cuda_bf16_p2_supports(node->src[0],node->src[1],node);
    }
    if (p2_serial_prefill) { GGML_ASSERT(cuda_ctx->curr_stream_no == 0); }
#endif

    const auto try_launch_concurrent_event = [&](const ggml_tensor * node) {
        if (p2_serial_prefill) { return; }
    ...
    while (!graph_evaluated_or_captured) {
        ...
        if (!use_cuda_graph || cuda_graph_update_required) {
            ...
            if (!p2_serial_prefill && stream_ctx.concurrent_events.size() > 0) {
                ...
                for (int j = 0; j < GGML_MAX_SRC; j++) {
                    if (node->src[j] != nullptr) {
                        ...
                        assert(node->src[j]->buffer->buft == ggml_backend_cuda_buffer_type(cuda_ctx->device) ||
                               ggml_cuda_bf16_p2_on_device(node->src[j], cuda_ctx->device) ||
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    for (int i=0;i<GGML_MAX_SRC;++i) {
        if (ggml_cuda_is_bf16_p2(op->src[i])) {
            return i == 0 && op->op == GGML_OP_MUL_MAT &&
                ggml_cuda_bf16_p2_on_device(op->src[0],dev_ctx->device) &&
                (ggml_cuda_bf16_p2_supports(op->src[0],op->src[1],op) || ggml_cuda_bf16_p2_can_materialize(op->src[0],op->src[1],op)) &&
...
static bool ggml_backend_cuda_device_supports_buft(ggml_backend_dev_t dev, ggml_backend_buffer_type_t buft) {
    ...
    return (((ggml_backend_buft_is_cuda(buft) || ggml_backend_buft_is_cuda_split(buft) || ggml_cuda_buft_is_bf16_p2(buft)) && buft->device == dev) || (integrated && ggml_backend_buft_is_cuda_host(buft)));
}
...
static void * ggml_backend_cuda_reg_get_proc_address(ggml_backend_reg_t reg, const char * name) {
    GGML_UNUSED(reg);
#ifdef GGML_CUDA_BF16_P2_AVAILABLE
    if (strcmp(name, "ggml_backend_cuda_bf16_p2_create_tensor") == 0) { return (void *)ggml_cuda_bf16_p2_create; }
    if (strcmp(name, "ggml_backend_cuda_bf16_p3_create_tensor") == 0) { return (void *)ggml_cuda_bf16_p3_create; }
    if (strcmp(name, "ggml_backend_cuda_bf16_p2_prepare_scratch") == 0) { return (void *)ggml_cuda_bf16_p2_prepare_scratch; }
#endif
