...
template <typename T, typename type_acc, int ncols_dst, int block_size, bool has_fusion = false, bool is_multi_token_id = false, bool rounded_swiglu = false, bool round_output_bf16 = false>
...
#pragma unroll
        ...
        if (use_gate) {
            ...
            switch (glu_op) {
                case GGML_GLU_OP_SWIGLU:
                    if constexpr (rounded_swiglu) {
                        // Match the unfused graph, including every F32->BF16->F32
                        // boundary. Do not replace this by ordinary fused SwiGLU.
                        const float g = __bfloat162float(__float2bfloat16(gate_value));
                        const float u = __bfloat162float(__float2bfloat16(value));
                        const float s = __bfloat162float(__float2bfloat16(ggml_cuda_op_silu_single(g)));
                        value = __bfloat162float(__float2bfloat16(s * u));
                    } else {
    ...
    if constexpr (round_output_bf16) {
        // Preserve the reference F32 dot product and both reductions; only fuse
        // the following explicit F32 -> BF16 -> F32 unary at the final store.
        static_assert(std::is_same_v<T, nv_bfloat16> && std::is_same_v<type_acc, float>);
        static_assert(ncols_dst == 1 && !has_fusion && !is_multi_token_id && !rounded_swiglu);
...
void ggml_cuda_mul_mat_vec_bf16_round_output(
        ggml_backend_cuda_context & ctx, const ggml_tensor * weight,
        const ggml_tensor * input, ggml_tensor * dst) {
#ifdef GGML_CUDA_BF16_P2_AVAILABLE
    if (ggml_cuda_is_bf16_p2(weight)) { ggml_cuda_bf16_p2_mul_mat(ctx, weight, input, dst, 1); return; }
#endif
    const int ncols = (int) weight->ne[0];
    const int nrows = (int) weight->ne[1];
    GGML_ASSERT(weight->type == GGML_TYPE_BF16 && input->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32);
    GGML_ASSERT(ncols > 0 && ncols % 512 == 0 && dst->ne[0] == nrows);
    GGML_ASSERT(ggml_nrows(input) == 1 && ggml_nrows(dst) == 1);
    GGML_ASSERT(ggml_is_contiguous(weight) && ggml_is_contiguous(input) && ggml_is_contiguous(dst));
    const uint3 one = init_fastdiv_values(1);
    const ggml_cuda_mm_fusion_args_device empty{};
    // For positive widths divisible by 512 the reference selector picks 256
    // threads. No paired accumulators, reassociation, or reduction changes.
    mul_mat_vec_f<nv_bfloat16, float, 1, 256, false, false, false, true>
        <<<dim3(nrows, 1, 1), dim3(256, 1, 1), 32*sizeof(float), ctx.stream()>>>(
            (const nv_bfloat16 *) weight->data, (const float *) input->data, nullptr,
            empty, (float *) dst->data, ncols/2, make_uint3(0, 0, 0), ncols, ncols/2, nrows,
            one, ncols*nrows, ncols, nrows,
            one, ncols*nrows, ncols, nrows, 0);
}

void ggml_cuda_mul_mat_vec_bf16_rounded_swiglu(
        ggml_backend_cuda_context & ctx, const ggml_tensor * packed_weight,
        const ggml_tensor * input, ggml_tensor * dst) {
#ifdef GGML_CUDA_BF16_P2_AVAILABLE
    if (ggml_cuda_is_bf16_p2(packed_weight)) { ggml_cuda_bf16_p2_mul_mat(ctx, packed_weight, input, dst, 2); return; }
#endif
    const int ncols = (int) packed_weight->ne[0];
    const int nrows = (int) dst->ne[0];
    GGML_ASSERT(packed_weight->type == GGML_TYPE_BF16 && input->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32);
    GGML_ASSERT(packed_weight->ne[1] == 2*nrows && ncols % 512 == 0);
    GGML_ASSERT(ggml_nrows(input) == 1 && ggml_nrows(dst) == 1);
    GGML_ASSERT(ggml_is_contiguous(packed_weight) && ggml_is_contiguous(input) && ggml_is_contiguous(dst));

    ggml_cuda_mm_fusion_args_device fusion{};
    fusion.gate = packed_weight->data;
    fusion.glu_op = GGML_GLU_OP_SWIGLU;
    const auto * up = (const nv_bfloat16 *) packed_weight->data + int64_t(nrows)*ncols;
    const uint3 one = init_fastdiv_values(1);
    // The original kernel selects 256 threads for these dimensions. Reuse its
    // per-thread dot products and two-level reductions verbatim, with separate
    // accumulators for paired rows and one shared load of the input vector.
    mul_mat_vec_f<nv_bfloat16, float, 1, 256, true, false, true>
        <<<dim3(nrows, 1, 1), dim3(256, 1, 1), 2*32*sizeof(float), ctx.stream()>>>(
            up, (const float *) input->data, nullptr, fusion, (float *) dst->data,
            ncols/2, make_uint3(0, 0, 0), ncols, ncols/2, nrows,
            one, ncols*nrows, ncols, nrows,
            one, ncols*nrows, ncols, nrows, 0);
}
#endif
...
void ggml_cuda_mul_mat_vec_f(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids, ggml_tensor * dst,
    ...
    if (ggml_cuda_is_bf16_p2(src0)) {
        GGML_ASSERT(!ids && !fusion); // General ggml GLU/bias fusion is not a P2 consumer.
        ggml_cuda_bf16_p2_mul_mat(ctx, src0, src1, dst, 0); return;
    }
#endif
