...
QwenDecoderLayerOutputs QwenDecoderLayerModule::build(
    ...
    if (weights.attention_scale.has_value()) {
        attn_out = LayerScaleModule{}.build(ctx, attn_out, *weights.attention_scale);
    }
    ...
    if (weights.ffn_scale.has_value()) {
        ff = LayerScaleModule{}.build(ctx, ff, *weights.ffn_scale);
    }
...
QwenDecoderLayerOutputs QwenDecoderLayerModule::build_static_cache_impl(
    ...
    if (config_.runtime.static_cache.update_mode == QwenDecoderStaticCacheUpdateMode::DirectSetRows) {
        ...
        if (config_.activation_cast.enabled && config_.activation_cast.after_static_cache_update) {
            const bool f16_flash_cache = config_.activation_cast.static_bf16_cache_to_f16 &&
                ctx.backend_type == core::BackendType::Cuda && flash_branches_allowed(config_) &&
                config_.runtime.attention.static_mode == QwenDecoderAttentionMode::FlashGroupedViewKV &&
                (attention_key_cache.type == GGML_TYPE_BF16 || attention_key_cache.type == GGML_TYPE_F16) &&
                attention_key_cache.type == attention_value_cache.type;
            if (f16_flash_cache) {
                if (attention_key_cache.type != GGML_TYPE_F16) {
                    attention_key_cache = core::wrap_tensor(ggml_cast(ctx.ggml, attention_key_cache.tensor, GGML_TYPE_F16),
                        attention_key_cache.shape, GGML_TYPE_F16);
                    attention_value_cache = core::wrap_tensor(ggml_cast(ctx.ggml, attention_value_cache.tensor, GGML_TYPE_F16),
                        attention_value_cache.shape, GGML_TYPE_F16);
                }
            } else {
