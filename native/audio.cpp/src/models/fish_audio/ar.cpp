...
struct RankedSemanticLogit {
    int32_t token = 0;
    float logit = 0.0F;
};

struct PreparedSemanticLogits {
    size_t source_size = 0;
    float max_logit = -std::numeric_limits<float>::infinity();
    double denom = 0.0;
    std::vector<RankedSemanticLogit> ranked;
};
...
int64_t compact_slow_logits_size(const FishAudioConfig & config);
...
ggml_type fish_cache_type(core::BackendType backend_type) {
    if (backend_type == core::BackendType::Vulkan) {
        return GGML_TYPE_F32;
    }
    // K/V projections and RoPE outputs are already BF16-rounded. CUDA flash
    // reads F16(BF16(x)) in the original path; persist those exact operands to
    // avoid converting the full cache on every step. CPU/HIP/Metal stay BF16.
    if (backend_type == core::BackendType::Cuda && std::getenv("FISH_DISABLE_F16_KV_STORAGE") == nullptr &&
        std::getenv("FISH_DISABLE_F16_ATTENTION_CACHE") == nullptr) {
...
modules::QwenDecoderActivationCastPolicy fish_activation_cast_policy(core::BackendType backend_type) {
    ...
    // Preserve every BF16 rounding boundary, but use the existing single-kernel
    // round trip on CUDA/HIP. Other backends retain their original policy.
    const auto * disable_fused_round = std::getenv("FISH_DISABLE_FUSED_BF16_ROUND");
    policy.fused_round = (backend_type == core::BackendType::Cuda || backend_type == core::BackendType::Hip) &&
        !(disable_fused_round != nullptr && std::strcmp(disable_fused_round, "1") == 0);
    ...
    policy.static_bf16_cache_to_f16 = backend_type == core::BackendType::Cuda &&
        std::getenv("FISH_DISABLE_F16_ATTENTION_CACHE") == nullptr;
...
modules::QwenCausalDecoderConfig make_slow_decoder_config(
    const FishAudioConfig & model_config,
    core::BackendType backend_type) {
    const auto & config = model_config.text;
    ...
    out.logits_size = compact_slow_logits_size(model_config);
...
modules::QwenCausalDecoderWeights bind_slow_weights(
    ...
    out.lm_head = binding::linear_data(constants, weights.slow_output);
    return out;
}
...
int64_t semantic_token_count(const FishAudioConfig & config) {
    if (config.semantic_start_token_id < 0 ||
        config.semantic_end_token_id < config.semantic_start_token_id ||
        config.semantic_end_token_id >= config.text.vocab_size) {
        throw std::runtime_error("Fish Audio semantic token range is outside the text vocabulary");
    }
    return config.semantic_end_token_id - config.semantic_start_token_id + 1;
}

bool eos_is_semantic(const FishAudioConfig & config) {
    return config.im_end_token_id >= config.semantic_start_token_id &&
        config.im_end_token_id <= config.semantic_end_token_id;
}

int64_t compact_slow_logits_size(const FishAudioConfig & config) {
    if (config.im_end_token_id < 0 || config.im_end_token_id >= config.text.vocab_size) {
        throw std::runtime_error("Fish Audio end token is outside the text vocabulary");
    }
    return semantic_token_count(config) + (eos_is_semantic(config) ? 0 : 1);
}

core::TensorValue make_compact_slow_output(
    core::BackendWeightStore & store,
    const assets::TensorSource & source,
    const FishAudioConfig & config,
    assets::TensorStorageType storage_type) {
    storage_type = store.resolve_loaded_storage_type(
        source, "embeddings.weight", storage_type,
        core::TensorShape::from_dims({config.text.vocab_size, config.text.dim}));
    const auto full = source.require_tensor(
        "embeddings.weight",
        storage_type,
        {config.text.vocab_size, config.text.dim});
    const size_t row_bytes = ggml_row_size(full.type, config.text.dim);
    const size_t expected_bytes = row_bytes * static_cast<size_t>(config.text.vocab_size);
    if (full.bytes.size() != expected_bytes) {
        throw std::runtime_error("Fish Audio compact LM head source byte size mismatch");
    }

    const int64_t semantic_count = semantic_token_count(config);
    const int64_t compact_rows = compact_slow_logits_size(config);
    std::vector<std::byte> compact(row_bytes * static_cast<size_t>(compact_rows));
    const size_t semantic_bytes = row_bytes * static_cast<size_t>(semantic_count);
    std::memcpy(
        compact.data(),
        full.bytes.data() + row_bytes * static_cast<size_t>(config.semantic_start_token_id),
        semantic_bytes);
    if (!eos_is_semantic(config)) {
        std::memcpy(
            compact.data() + semantic_bytes,
            full.bytes.data() + row_bytes * static_cast<size_t>(config.im_end_token_id),
            row_bytes);
    }
    return store.make_tensor(
        core::TensorShape::from_dims({compact_rows, config.text.dim}),
        full.type,
        compact.data(),
        compact.size());
}
...
FishARWeights load_ar_weights(
    ...
    weights.slow_output = make_compact_slow_output(
        *weights.store,
        source,
        config,
        storage_type);
    ...
    weights.fast_output = weights.store->load_tensor(
        ...
        storage_type,
        {config.fast.vocab_size, config.fast.dim});
    // Independent staged opt-ins. Fast-AR is exclusively single-token; Slow-AR
    // additionally requires a preallocated bounded raw prefill adapter.
    const char * p2 = std::getenv("FISH_AUDIO_EXPERIMENTAL_BF16_P2");
    const char * p3 = std::getenv("FISH_AUDIO_EXPERIMENTAL_BF16_P3");
    const char * p2_slow = std::getenv("FISH_AUDIO_EXPERIMENTAL_BF16_P2_SLOW");
    const bool p3_fast_enabled=p3 && std::atoi(p3)!=0;
    const bool p2_fast_enabled=(p2 && std::atoi(p2)!=0) || p3_fast_enabled, p2_slow_enabled=p2_slow && std::atoi(p2_slow)!=0;
    if (backend_type == core::BackendType::Cuda && (p2_fast_enabled || p2_slow_enabled)) {
        const auto device = ggml_backend_get_device(backend);
        const auto reg = ggml_backend_dev_backend_reg(device);
        const auto factory = reinterpret_cast<core::BackendWeightStore::StorageFactory>(
            ggml_backend_reg_get_proc_address(reg, "ggml_backend_cuda_bf16_p2_create_tensor"));
        if (factory) {
            const auto factory_p3 = reinterpret_cast<core::BackendWeightStore::StorageFactory>(
                ggml_backend_reg_get_proc_address(reg,"ggml_backend_cuda_bf16_p3_create_tensor"));
            const auto fast_factory=p3_fast_enabled && factory_p3?factory_p3:factory;
            auto mark_layer = [&](const auto & layer,core::BackendWeightStore::StorageFactory chosen) {
                for (const auto * value : {&layer.qkv_proj,&layer.o_proj,&layer.down_proj,&layer.gate_up_proj}) {
                    weights.store->set_storage_factory(value->tensor,chosen);
                }
            };
            if (p2_fast_enabled) {
                for (const auto & layer : weights.fast_layers) { mark_layer(layer,fast_factory); }
                weights.store->set_storage_factory(weights.fast_output.tensor,fast_factory);
            }
            if (p2_slow_enabled) {
                size_t max_bytes=0;
                for (const auto & layer : weights.slow_layers) {
                    for (const auto * value : {&layer.qkv_proj,&layer.o_proj,&layer.down_proj,&layer.gate_up_proj}) {
                        if (value->tensor->type == GGML_TYPE_BF16) { max_bytes=std::max(max_bytes,ggml_nbytes(value->tensor)); }
                    }
                }
                const auto prepare=reinterpret_cast<bool (*)(ggml_backend_t,size_t)>(
                    ggml_backend_reg_get_proc_address(reg,"ggml_backend_cuda_bf16_p2_prepare_scratch"));
                if (prepare && max_bytes && prepare(backend,max_bytes)) {
                    for (const auto & layer : weights.slow_layers) { mark_layer(layer,factory); }
                }
            }
        }
    }
...
PreparedSemanticLogits prepare_semantic_logits(
    const FishAudioConfig & config,
    int32_t im_end_id,
    const std::vector<float> & logits,
    int top_k) {
    if (im_end_id != config.im_end_token_id) {
        throw std::runtime_error("Fish Audio tokenizer/config end token mismatch");
    }
    const int64_t semantic_count = semantic_token_count(config);
    if (static_cast<int64_t>(logits.size()) != compact_slow_logits_size(config)) {
        throw std::runtime_error("Fish Audio compact slow logits size mismatch");
    }

    std::vector<int32_t> order;
    order.reserve(logits.size());
    float max_logit = -std::numeric_limits<float>::infinity();
    const auto append_if_finite = [&](int32_t local_index) {
        const float logit = logits[static_cast<size_t>(local_index)];
        if (std::isfinite(logit)) {
            order.push_back(local_index);
            max_logit = std::max(max_logit, logit);
        }
    };
    if (!eos_is_semantic(config) && im_end_id < config.semantic_start_token_id) {
        append_if_finite(static_cast<int32_t>(semantic_count));
    }
    for (int64_t i = 0; i < semantic_count; ++i) {
        append_if_finite(static_cast<int32_t>(i));
    }
    if (!eos_is_semantic(config) && im_end_id > config.semantic_end_token_id) {
        append_if_finite(static_cast<int32_t>(semantic_count));
    }

    double denom = 0.0;
    for (const int32_t local_index : order) {
        denom += std::exp(logits[static_cast<size_t>(local_index)] - max_logit);
    }
    if (!(denom > 0.0)) {
        throw std::runtime_error("Fish Audio semantic logits produced zero probability mass");
    }
    ...
    if (candidate_count < order.size()) {
        std::partial_sort(
            order.begin(),
            order.begin() + static_cast<std::ptrdiff_t>(candidate_count),
            order.end(),
            by_logit_desc);
        order.resize(candidate_count);
    } else {
    ...
    PreparedSemanticLogits prepared;
    prepared.source_size = static_cast<size_t>(config.text.vocab_size);
    prepared.max_logit = max_logit;
    prepared.denom = denom;
    prepared.ranked.reserve(order.size());
    for (const int32_t local_index : order) {
        const int32_t token = local_index < semantic_count
            ? static_cast<int32_t>(config.semantic_start_token_id + local_index)
            : im_end_id;
        prepared.ranked.push_back({token, logits[static_cast<size_t>(local_index)]});
    }
    return prepared;
}

SampleDistribution semantic_distribution(
    const PreparedSemanticLogits & prepared,
    float temperature,
    float top_p) {
    double cumulative = 0.0;
    std::vector<SampleCandidate> kept;
    kept.reserve(prepared.ranked.size());
    for (size_t i = 0; i < prepared.ranked.size(); ++i) {
        const auto & ranked = prepared.ranked[i];
        const float probability = static_cast<float>(
            std::exp(ranked.logit - prepared.max_logit) / prepared.denom);
        cumulative += probability;
        const bool remove = cumulative > static_cast<double>(top_p) && i != 0;
        if (!remove) {
            kept.push_back({ranked.token, 0.0F});
        }
    }
    ...
    for (size_t i = 0; i < kept.size(); ++i) {
        filtered_max = std::max(filtered_max, prepared.ranked[i].logit / temperature_scale);
    }
    double filtered_denom = 0.0;
    for (size_t i = 0; i < kept.size(); ++i) {
        kept[i].probability = std::exp(prepared.ranked[i].logit / temperature_scale - filtered_max);
        filtered_denom += kept[i].probability;
    }
    if (!(filtered_denom > 0.0)) {
        throw std::runtime_error("Fish Audio semantic sampling filter produced zero probability mass");
    }
    for (auto & candidate : kept) {
        candidate.probability = static_cast<float>(
            static_cast<double>(candidate.probability) / filtered_denom);
    }
    return {prepared.source_size, std::move(kept)};
}
...
public:
    ...
    // Host storage must stay alive through the next graph compute (which
    // synchronizes). CUDA copies and graph work use the same backend stream.
    void upload_step_input(ggml_tensor * tensor, const void * data, size_t offset, size_t size) const {
        if (backend_type_ == core::BackendType::Cuda && async_upload_) {
            ggml_backend_tensor_set_async(backend_, tensor, data, offset, size);
        } else {
...
private:
    bool async_upload_ = std::getenv("FISH_DISABLE_ASYNC_UPLOAD") == nullptr;
...
public:
    ...
    engine::codecs::FishDacCodes generate(
        ...
        const FishAudioARRuntime::StreamCallback & stream_callback) {
        ...
        // Optional verification-only trace. Records every sampled semantic/code
        // frame (including EOS/discarded final frames), plus returned prefill
        // tensors. No RNG call or arithmetic changes; disabled during timing.
        std::ofstream parity_trace;
        if (const char * path = std::getenv("FISH_AUDIO_DEBUG_TRACE_FILE"); path && *path) {
            parity_trace.open(path,std::ios::binary|std::ios::app);
            if (!parity_trace) { throw std::runtime_error("cannot open Fish parity trace"); }
        }
        auto trace_record = [&](uint32_t tag,const void * data,size_t bytes) {
            if (!parity_trace.is_open()) { return; }
            const uint32_t header[3]={0x50325431u,tag,static_cast<uint32_t>(bytes)};
            parity_trace.write(reinterpret_cast<const char *>(header),sizeof(header));
            if (bytes) { parity_trace.write(static_cast<const char *>(data),static_cast<std::streamsize>(bytes)); }
            if (!parity_trace) { throw std::runtime_error("cannot write Fish parity trace"); }
        };
        trace_record(1,prefill.forward.logits.data(),prefill.forward.logits.size()*sizeof(float));
        trace_record(2,prefill.forward.hidden.data(),prefill.forward.hidden.size()*sizeof(float));
        ...
        int64_t last_streamed_frames = 0;
        auto make_codes = [&](int64_t frames) {
        ...
        auto emit_stream_prefix = [&](bool final) {
            if (!stream_callback) {
                return;
            }
            int64_t stable_frames =
                static_cast<int64_t>(generated_frame_major.size()) / assets.config.fast.num_codebooks;
            // Until EOS is known, the newest AR frame may be removed by the
            // max-token guard below. Holding it back keeps every emitted PCM
            // sample part of the final result.
            if (!final && stable_frames > 0) {
                --stable_frames;
            }
            if (stable_frames <= 0) {
                return;
            }
            if (!final && stable_frames - last_streamed_frames < frames_per_chunk) {
                return;
            }
            // A final notification is useful even when the last regular chunk
            // already covered the complete prefix: the codec layer can retain
            // that exact full-prefix decode as the saved offline result.
            const auto codes = make_codes(stable_frames);
            stream_callback(codes, final);
            last_streamed_frames = stable_frames;
        };
        auto frame = sample_frame(prefill.forward.logits, prefill.forward.hidden, options, sample, false, profile);
        trace_record(3,frame.data(),frame.size()*sizeof(int32_t));
        if (frame.front() == im_end_id()) {
            trace_record(0,nullptr,0);
            log_profile(profile);
...
private:
    ...
    public:
        PrefillGraph(
            ...
            auto decoder = build_fish_causal_decoder(
                ...
                *constants_,
                ...
                make_slow_decoder_config(assets.config, runtime_->backend_type()),
        ...
        SlowPrefillOutput run(const std::vector<float> & embeddings, FishARProfile & profile) {
            ...
            out.forward.logits.resize(static_cast<size_t>(ggml_nelements(logits_)));
    ...
    public:
        StepGraph(std::shared_ptr<const FishARWeightsRuntime> runtime, int64_t cache_steps)
            ...
            const ggml_type cache_type = fish_cache_type(runtime_->backend_type());
        ...
        SlowForwardOutput run(const std::vector<float> & embedding, FishARProfile & profile) {
            ...
            runtime_->upload_step_input(position_, &pos, 0, sizeof(pos));
            const int32_t cache_slot = static_cast<int32_t>(cache_.valid_steps());
            runtime_->upload_step_input(cache_slot_, &cache_slot, 0, sizeof(cache_slot));
            const auto visible = ggml_fp32_to_fp16(0.0F);
            mask_scratch_[static_cast<size_t>(cache_.valid_steps())] = visible;
            runtime_->upload_step_input(
                mask_,
                &visible,
            ...
            timing_start = Clock::now();
            runtime_->upload_step_input(input_, embedding.data(), 0, embedding.size() * sizeof(float));
            ...
            out.logits.resize(static_cast<size_t>(ggml_nelements(logits_)));
    ...
    public:
        explicit FastGraph(std::shared_ptr<const FishARWeightsRuntime> runtime)
            ...
            // Codebook zero comes from the semantic token. Its initial Fast-AR
            // pass only needs to populate every layer's K/V state. Reuse the
            // exact nodes and allocations of the normal graph, but traverse only
            // cache-write dependencies: the final layer's attention/MLP and the
            // output head cannot affect any later codebook at this position.
            initial_graph_ = ggml_new_graph_custom(graph_ctx_.get(), 32768, false);
            int cache_writes = 0;
            for (int i = 0; i < ggml_graph_n_nodes(graph_); ++i) {
                auto * node = ggml_graph_node(graph_, i);
                if (node->op == GGML_OP_SET_ROWS) {
                    ggml_build_forward_expand(initial_graph_, node);
                    ++cache_writes;
                }
            }
            if (cache_writes != 2 * config.n_layer) {
                throw std::runtime_error("Fish Audio initial graph must update every K/V cache");
            }
            // ggml's CUDA graph cache is keyed by the first node address. Both
            // graphs share allocations, but need distinct cache identities or
            // switching between them would invalidate capture every frame.
            initial_entry_ = *ggml_graph_node(initial_graph_, 0);
            ggml_graph_nodes(initial_graph_)[0] = &initial_entry_;
            engine::debug::trace_log_scalar("fish_audio.fast.full_graph_nodes", ggml_graph_n_nodes(graph_));
            engine::debug::trace_log_scalar("fish_audio.fast.initial_graph_nodes", ggml_graph_n_nodes(initial_graph_));
...
#endif
            core::release_backend_graph_resources(runtime_->backend(), graph_);
            core::release_backend_graph_resources(runtime_->backend(), initial_graph_);
            if (gallocr_ != nullptr) {
                ggml_gallocr_free(gallocr_);
            }
            if (state_buffer_ != nullptr) {
...
#endif
            {
                runtime_->upload_step_input(position_, &initial_position, 0, sizeof(initial_position));
                const auto visible = ggml_fp32_to_fp16(0.0F);
                std::fill(mask_scratch_.begin(), mask_scratch_.end(), ggml_fp32_to_fp16(-INFINITY));
                mask_scratch_[0] = visible;
                runtime_->upload_step_input(mask_, mask_scratch_.data(), 0, mask_scratch_.size() * sizeof(ggml_fp16_t));
            }
            profile.fast_mask_upload_ms += engine::debug::elapsed_ms(timing_start, Clock::now());
            timing_start = Clock::now();
            runtime_->upload_step_input(input_, input.data(), 0, input.size() * sizeof(float));
            profile.fast_input_upload_ms += engine::debug::elapsed_ms(timing_start, Clock::now());
            core::set_backend_threads(runtime_->backend(), runtime_->threads());
            timing_start = Clock::now();
            const auto * disable_prune = std::getenv("FISH_DISABLE_FAST_INITIAL_PRUNE");
            auto * graph = disable_prune != nullptr && std::strcmp(disable_prune, "1") == 0 ? graph_ : initial_graph_;
            const ggml_status status = core::compute_backend_graph(runtime_->backend(), graph, nullptr, "fish_audio.ar.fast_initial");
    ...
    private:
        ...
        std::vector<ggml_fp16_t> mask_scratch_;
        ggml_cgraph * graph_ = nullptr;
        ggml_cgraph * initial_graph_ = nullptr;
        ggml_tensor initial_entry_{};
        ggml_gallocr_t gallocr_ = nullptr;
        ggml_backend_buffer_t state_buffer_ = nullptr;
#ifdef ENGINE_HAS_HIP_FISH_FAST_SAMPLER
    ...
    std::vector<int32_t> sample_frame(
        ...
        const auto & weights = runtime_->weights();
        auto timing_start = Clock::now();
        const auto prepared = prepare_semantic_logits(config, im_end_id(), slow_logits, options.top_k);
        profile.sample_bias_ms += engine::debug::elapsed_ms(timing_start, Clock::now());
        timing_start = Clock::now();
        int32_t main_token = sample_from_distribution(
            semantic_distribution(prepared, options.temperature, options.top_p),
            sample,
            sampling_policy_);
        profile.sample_main_ms += engine::debug::elapsed_ms(timing_start, Clock::now());
        timing_start = Clock::now();
        const int32_t high_token = sample_from_distribution(
            semantic_distribution(prepared, kRasHighTemperature, kRasHighTopP),
            sample,
            sampling_policy_);
...
#endif
        // The first Fast-AR output is never sampled: codebook 0 is derived directly
        // from the semantic token. Execute the state update without copying 4096
        // unused logits back to the host.
...
engine::codecs::FishDacCodes FishAudioARRuntime::generate(
    const FishAudioPrompt & prompt,
    const FishAudioGenerationOptions & options) {
    return impl_->generate(prompt, options, 0, {});
}

engine::codecs::FishDacCodes FishAudioARRuntime::generate_streaming(
    const FishAudioPrompt & prompt,
    const FishAudioGenerationOptions & options,
    int64_t frames_per_chunk,
    ...
    if (!callback) {
        throw std::runtime_error("Fish Audio streaming requires a callback");
    }
    return impl_->generate(prompt, options, frames_per_chunk, callback);
}
