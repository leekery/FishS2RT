...
constexpr int64_t kDefaultStreamChunkFrames = 8;
...
bool eager_reference_cache_from_options(const runtime::SessionOptions & options) {
    if (const auto value = runtime::find_option(
            options.options,
            {"fish_audio.eager_reference_cache", "eager_reference_cache"})) {
        return runtime::parse_bool_option(*value, "fish_audio.eager_reference_cache");
    }
    return false;
}
...
FishAudioSession::FishAudioSession(
    ...
    if (task_.task != runtime::VoiceTaskKind::Tts ||
        ...
        throw std::runtime_error("Fish Audio supports offline or streaming TTS sessions");
    }
...
void FishAudioSession::prepare(const runtime::SessionPreparationRequest & request) {
    ...
    if (defaults_.has_value() && eager_reference_cache_from_options(options())) {
        for (const auto & reference : defaults_->references) {
            (void) resolve_reference_codes(reference);
        }
    }
    mark_prepared();
}
...
runtime::StreamingPolicy FishAudioSession::streaming_policy() const {
    ...
    // Fish generation is synchronous internally, but it pushes decoded audio
    // events from the AR loop through stream_event_sink_.
...
void FishAudioSession::start_stream(const runtime::TaskRequest & request) {
    require_prepared("Fish Audio start_stream()");
    if (task_.mode != runtime::RunMode::Streaming) {
        throw std::runtime_error("Fish Audio start_stream() requires a streaming session");
    }
    if (stream_started_) {
        throw std::runtime_error("Fish Audio streaming is already active");
    }

    const int64_t stream_chunk_frames = runtime::parse_i64_option(
        request.options,
        {"fish_audio.stream_chunk_frames", "stream_chunk_frames"})
        .value_or(kDefaultStreamChunkFrames);
    if (stream_chunk_frames <= 0 || stream_chunk_frames > 256) {
        throw std::runtime_error("Fish Audio stream_chunk_frames must be in [1, 256]");
    }

    stream_started_ = true;
    stream_result_.reset();
    stream_event_sequence_ = 0;
    stream_started_at_ = Clock::now();
    try {
        const bool mem_saver = mem_saver_from_options(options());
        const auto request_options = generation_options_from_request(request);
        const auto text_chunk_mode =
            engine::text::parse_text_chunk_mode_override(request.options).value_or(engine::text::TextChunkMode::Default);
        const auto chunk_requests = runtime::chunk_text_request(request, request_options.text_chunk_size, text_chunk_mode);
        engine::debug::trace_log_scalar("fish_audio.streaming.frames_per_event", stream_chunk_frames);
        engine::debug::trace_log_scalar("fish_audio.text_chunk_count", static_cast<int64_t>(chunk_requests.size()));

        runtime::AudioBuffer merged_audio;
        std::vector<engine::codecs::FishDacCodes> reference_codes;
        std::vector<engine::codecs::FishDacCodes> generated_segments;
        generated_segments.reserve(chunk_requests.size());
        double codec_stream_decode_total_ms = 0.0;
        double codec_final_decode_total_ms = 0.0;
        std::optional<FishAudioConversationTurn> previous_turn = std::nullopt;
        for (size_t chunk_index = 0; chunk_index < chunk_requests.size(); ++chunk_index) {
            const auto & chunk_request = chunk_requests[chunk_index];
            auto fish_request = make_request(chunk_request);
            ...
            auto generated = generator_->generate_streaming(
                fish_request,
                reference_codes,
                previous_turn,
                mem_saver,
                ...
                [&](const runtime::AudioBuffer & audio) {
                    ...
                    named.id = "stream_" + std::to_string(stream_event_sequence_++);
                    named.audio = audio;
                    named.meta.emplace("text_chunk", std::to_string(chunk_index));
                    event.named_audio_outputs.push_back(std::move(named));
                    stream_event_sink_(event);
                },
                true);
            codec_stream_decode_total_ms += generated.codec_stream_decode_ms;
            generated_segments.push_back(generated.codes);
            if (chunk_requests.size() > 1) {
                previous_turn = FishAudioConversationTurn{fish_request.text, std::move(generated.codes)};
            }
        }
        // Exact-length canonical renders are deliberately deferred until all
        // live segments have been delivered.  Otherwise a full decode of one
        // text segment stalls generation of the next and can drain playback.
        for (const auto & codes : generated_segments) {
            const auto decode_start = Clock::now();
            runtime::append_audio_buffer(merged_audio, generator_->decode_generated_codes(codes));
            codec_final_decode_total_ms += engine::debug::elapsed_ms(decode_start, Clock::now());
        }
        engine::debug::timing_log_scalar(
            "fish_audio.codec_stream_decode_total_ms",
            codec_stream_decode_total_ms);
        engine::debug::timing_log_scalar(
            "fish_audio.codec_final_decode_total_ms",
            codec_final_decode_total_ms);
        engine::debug::timing_log_scalar(
            "fish_audio.codec_decode_total_ms",
            codec_stream_decode_total_ms + codec_final_decode_total_ms);
        runtime::TaskResult result;
        result.audio_output = std::move(merged_audio);
        stream_result_ = std::move(result);
    } catch (...) {
        stream_result_.reset();
...
runtime::TaskResult FishAudioSession::finish_stream() {
    if (!stream_started_ || !stream_result_.has_value()) {
        throw std::runtime_error("Fish Audio streaming has not completed");
    }
    auto result = std::move(*stream_result_);
    stream_result_.reset();
    stream_started_ = false;
    engine::debug::timing_log_scalar("session.wall_ms", engine::debug::elapsed_ms(stream_started_at_, Clock::now()));
    return result;
}
...
runtime::StreamEvent FishAudioSession::process_audio_chunk(const runtime::AudioChunk & chunk) {
    (void)chunk;
    throw std::runtime_error("Fish Audio TTS streaming does not consume audio chunks");
}
