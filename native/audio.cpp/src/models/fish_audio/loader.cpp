...
runtime::ModelCliInterface cli(const FishAudioAssets &) {
    ...
    out.request_options = {
        ...
        {"fish_audio.stream_chunk_frames", "N", "Streaming PCM cadence in codec frames; default 8 (~372 ms of audio)."},
    };
    out.session_options = {
        ...
        {"fish_audio.eager_reference_cache", "true|false", "Encode prepared reference audio during prepare(); default false."},
...
std::unique_ptr<runtime::IVoiceTaskSession> FishAudioLoadedModel::create_task_session(
    ...
    if (task.task != runtime::VoiceTaskKind::Tts ||
        ...
        throw std::runtime_error("Fish Audio S2-Pro supports offline or streaming TTS sessions");
    }
