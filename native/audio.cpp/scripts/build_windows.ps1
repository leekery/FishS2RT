...
function Get-PresetSettings {
    ...
    switch ($Name) {
        ...
        default {
            throw "Unsupported Windows preset '$Name'. Use windows-cpu-release, windows-vulkan-release, windows-vulkan-debug, windows-cuda-release, windows-cuda-portable-release, windows-cuda-debug, or windows-cuda-native-debug."
        }
    }
}
...
$fishS2RTNativeTestsValue = if ($FishS2RTNativeTests) { "ON" } else { "OFF" }
...
Write-Host "FishS2RT native regression tests: $fishS2RTNativeTestsValue"
...
$configureArgs = @(
    ...
    "-DFISHS2RT_BUILD_NATIVE_TESTS=$fishS2RTNativeTestsValue",
