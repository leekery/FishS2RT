...
struct QwenDecoderActivationCastPolicy {
    ...
    // CUDA flash attention consumes F16 K/V. With an already-BF16 cache this
    // avoids intermediate F32 widening; an F16 cache can be read directly.
    // Caller must BF16-round every K/V write before conversion to F16.
    // Opt-in: do not apply to manual attention or unrounded cache storage.
    bool static_bf16_cache_to_f16 = false;
...
struct QwenDecoderLayerWeights {
    ...
    QwenMLPWeights mlp;
    // Optional residual branch scales used by codec-style Qwen blocks.  They
    // are deliberately optional so existing language-model call sites retain
    // their exact graph and arithmetic.
    std::optional<LayerScaleWeights> attention_scale;
    std::optional<LayerScaleWeights> ffn_scale;
