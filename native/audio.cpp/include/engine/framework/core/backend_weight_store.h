...
public:
    ...
    ~BackendWeightStore() {
        ...
        for (auto buffer : dedicated_buffers_) {
            ggml_backend_buffer_free(buffer);
        }
    }

    // Optional explicit per-weight storage experiment. A failed factory must
    // leave the tensor untouched. Preallocated weights are skipped by ggml's
    // context allocator, so this does not retain a duplicate raw device arena.
    using StorageFactory = ggml_backend_buffer_t (*)(ggml_backend_t, ggml_tensor *, const void *, size_t);
    void set_storage_factory(ggml_tensor * tensor, StorageFactory factory) {
        for (auto & upload : pending_) {
            if (upload.tensor == tensor) { upload.factory = factory; return; }
        }
        throw std::runtime_error(name_ + " storage factory requires a pending tensor");
    }
    ...
    // Derived views/copies of a loaded weight must use the same backend storage
    // policy as load_tensor (notably native BF16 on Vulkan and Metal).
    assets::TensorStorageType resolve_loaded_storage_type(
        const assets::TensorSource & source,
        std::string_view tensor_name,
        assets::TensorStorageType storage_type,
        const TensorShape & shape) const {
        return type_for_storable_loaded_tensor(source, tensor_name, storage_type, shape);
    }
    ...
    void upload() {
        ...
        dedicated_buffers_.reserve(pending_.size());
        for (auto & upload : pending_) {
            if (!upload.factory || upload.tensor->type != GGML_TYPE_BF16) { continue; }
            ggml_backend_buffer_t dedicated = nullptr;
            if (upload.kind == PendingUploadKind::Bytes) {
                dedicated = upload.factory(backend_, upload.tensor, upload.bytes.data(), upload.bytes.size());
            } else if (upload.kind == PendingUploadKind::Tensor) {
                const auto raw = upload.source->require_tensor(upload.name, upload.storage_type, upload.expected_shape);
                dedicated = upload.factory(backend_, upload.tensor, raw.bytes.data(), raw.bytes.size());
            }
            if (dedicated) { dedicated_buffers_.push_back(dedicated); }
        }
        buffer_ = ggml_backend_alloc_ctx_tensors(ctx_.get(), backend_);
        if (buffer_ == nullptr) {
            for (const auto & upload : pending_) {
                if (!upload.tensor->data) {
        ...
        for (auto & upload : pending_) {
            if (upload.tensor->buffer != buffer_ && upload.factory && upload.tensor->data) {
                // Allocation can be a multi-buffer; identify only buffers we own
                // separately, rather than assuming all ordinary tensors share it.
                bool dedicated = false;
                for (auto b : dedicated_buffers_) { dedicated |= upload.tensor->buffer == b; }
...
private:
    ...
    std::vector<ggml_backend_buffer_t> dedicated_buffers_;
};
